...
// Cria (ou atualiza) um usuário PALESTRANTE vinculado ao evento. Ao criar, marca
// must_change_password=true e grava o hash da senha provisória. Se já existir,
// atualiza nome/evento e mantém a senha atual (não reseta quem já tem acesso).
// Retorna { ok, created } — created=true só na 1ª vez (para disparar o e-mail).
export async function upsertPalestranteUser({ email, name, eventoId, passwordHash }) {
  const e = String(email || "").trim().toLowerCase();
  if (!e) return { ok: false, created: false };
  const { rows } = await query(
    `INSERT INTO app_users (email, name, password_hash, role, active, must_change_password, evento_id)
     VALUES ($1, $2, $3, 'palestrante', true, true, $4)
    ...
    [e, name || e.split("@")[0], passwordHash, eventoId || null]
  );
  return { ok: true, created: !!(rows[0] && rows[0].inserted === true) };
}
...
// Regras da nova senha na troca (padrão de segurança). Pura (testável).
// Exige: 8+ caracteres, maiúscula, minúscula, número e caractere especial,
// e ser diferente da senha atual. Devolve { ok, error }.
export function validatePasswordChange({ currentPassword, newPassword }) {
  ...
  if (!/[A-Z]/.test(pw)) {
    return { ok: false, error: "A nova senha deve conter ao menos uma letra maiúscula." };
  }
  if (!/[a-z]/.test(pw)) {
    return { ok: false, error: "A nova senha deve conter ao menos uma letra minúscula." };
  }
  if (!/[0-9]/.test(pw)) {
    return { ok: false, error: "A nova senha deve conter ao menos um número." };
  }
  if (!/[^A-Za-z0-9]/.test(pw)) {
    return { ok: false, error: "A nova senha deve conter ao menos um caractere especial." };
  }
